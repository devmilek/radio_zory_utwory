        ...
        >
          <div className="flex flex-col gap-1">
            <h4 id={labelId} className="font-medium leading-none">
              {filters.length > 0 ? "Filtry" : "Dodaj filtry"}
            </h4>
            <p
              id={descriptionId}
              className={cn(
            ...
            >
              {filters.length > 0
                ? "Modyfikuj lub usuń istniejące filtry."
                : "Wybierz kolumnę, operator i wartość, aby dodać filtr."}
            </p>
          </div>
          {filters.length > 0 ? (
    ...
    <SortableItem value={filter.filterId} asChild>
      ...
      >
        <div className="min-w-[72px] text-center">
          {index === 0 ? (
            <span className="text-muted-foreground text-sm">Gdzie</span>
          ) : index === 1 ? (
            <Select
              value={joinOperator}
        ...
        <Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
          ...
          >
            <Command>
              <CommandInput placeholder="Szukaj kolumny..." />
              <CommandList>
                <CommandEmpty>Nie znaleziono kolumn</CommandEmpty>
                <CommandGroup>
                  {columns.map((column) => (
                    <CommandItem
                      key={column.id}
